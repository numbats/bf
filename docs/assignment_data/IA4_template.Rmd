...
*Use the last two years of your data as a test set. Generate forecasts for this period, from the models considered "best" in all assignments. More specifically, generate forecasts from the best benchmark, the best ETS and best ARIMA model. Plot the forecasts (both point forecasts and prediction intervals) together with the observed data and comment on these. (Make sure you can clearly visualise these. You may choose to plot on multiple graphs.) (6 marks)* 
...
*Evaluate the accuracy of the point forecasts over the test set. A table with accuracy measures will be necessary to be presented here. Comment on which forecasts are the most accurate. (4 marks)*
```{r}
...
*Use all three models to forecast the next 24-months (after the test set). Generate the necessary plots and comment on the forecasts. Make sure you can clearly visualise these. How have your models fared amidst the effects of COVID-19? (9 marks)*
